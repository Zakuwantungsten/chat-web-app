...
- Persists token in localStorage for session management
- Automatically verifies token on app initialization
...
**Backend: Socket.io Event Handlers (socketHandler.js)**
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('send_message', async (data) => {
      try {
        ...
        io.to(`room_${roomId}`).emit('new_message', messageWithSender);
...
- Manages room-based communication for chat rooms
- Broadcasts messages to all room members in real-time
- Implements typing indicators for enhanced UX
- Handles message persistence and user notifications

**Frontend: Message Sending (ChatInterface.jsx)**
...
- Emits message via Socket.io for instant delivery
- Clears input fields after successful send
...
**Backend: Multer Configuration (fileRoutes.js)**
...
**Explanation:**
- Uses Multer for secure file handling
- Generates unique filenames to prevent conflicts
- Validates file types against whitelist
- Enforces 50MB file size limit
- Creates database record for file metadata
- Implements cleanup on upload failure
